...
def _job_receiver_discovery() -> None:
    """Refresh public receiver directories without activating new candidates."""
    try:
        from core.radio.discovery import refresh_discovery

        snapshot = refresh_discovery()
        logger.info(
            "Scheduler: receiver discovery refreshed %s candidate(s)",
            snapshot.get("catalogued", 0),
        )
    except Exception:
...
def _job_darkship_cue_refresh() -> None:
    """Drain one bounded enrichment queue, prioritising current Live gaps."""
    try:
        from core.mda.watch import mda_watch
        ...
        retrospective = mda_watch.refresh_darkship_cues(
            limit=2,
            min_age_hours=72.0,
            max_age_days=10,
            recheck_hours=24.0,
        ...
        if (
            live["refreshed"] or live["with_unmatched_sar"]
            or retrospective["refreshed"] or retrospective["with_unmatched_sar"]
        ):
            logger.info(
                "Scheduler darkship queue: live scanned=%d refreshed=%d sar=%d; "
                "retro scanned=%d refreshed=%d sar=%d; hypotheses=%d",
                live["scanned"], live["refreshed"], live["with_unmatched_sar"],
                retrospective["scanned"], retrospective["refreshed"],
                retrospective["with_unmatched_sar"],
                live["hypotheses_evaluated"] + retrospective["hypotheses_evaluated"],
            )
    except Exception as exc:
...
def start() -> None:
    ...
    with _scheduler_lock:
        ...
        scheduler.add_job(_job_receiver_discovery, IntervalTrigger(hours=6),
