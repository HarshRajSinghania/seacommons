...
## 2026-09-21 — Live pipeline funnel audit + automatic Maritime hypothesis publication

Plan: `docs/superpowers/plans/2026-09-21-live-pipeline-funnel-audit.md`.

Audit finding: the real reason raw ingestion volume rarely turns into a public Maritime Intelligence Live item was a **wiring gap, not an evidence gate**. `core/review/maritime.py::apply_maritime_review()` — the function that performs `review_ready → assessed → published` — was fully implemented and unit-tested (`tests/test_maritime_review_apply.py`) but never called from any HTTP route (`grep -rn "core.review" apps/api/core/api/routes/*.py` → zero matches). `core/intel/hypothesis_engine.py::evaluate_episode()` only ever auto-transitioned a hypothesis up to `review_ready`; nothing moved it further, so every corroborated Maritime hypothesis was stuck forever, visible in Play/the operator dashboard but never on public Live. This traces to the "Review v0 / publication controls" packet (closed at `19dbb7d`), which built and accepted the domain logic ("Maritime approvals delegate to the existing hypothesis state machine") without ever wiring the route on top of it.

Product decision (explicit, this session): the pipeline should be fully automatic — no human approve/reject step. This is a deliberate departure from `docs/fixes.md` §M14.6's original framing ("analyst review/publish — the only path past candidate/collecting"), authorized directly by the product owner rather than assumed.

Implemented: `hypothesis_engine.py::evaluate_episode()` now continues the same automatic transition chain it already performed for `candidate → collecting → review_ready` on through `→ assessed → published`, gated by the **same, unmodified** `can_publish()` evidence bar (`core/intel/hypothesis.py`) — independent-lineage corroboration, non-empty reason codes, ≥2 distinct evidence links, no unresolved blocking identity conflict. No evidence gate, episode threshold, or corroboration rule was loosened; only the actor that performs the last two transitions changed, from a (nonexistent) human click to the same `hypothesis_engine_v1` system actor already recorded for every earlier transition — audit history stays honest about that. `has_unresolved_blocking_identity_conflict` / `allegation_shaped_wording` are never set `True` anywhere in production code today (checked), so `can_publish()` cannot yet block on either; if a future hypothesis type sets `allegation_shaped_wording`, this same automatic path will correctly stop at `assessed` and require a human `explicit_review_done` without any further change. Humanitarian review (`core/review/humanitarian.py::apply_humanitarian_review`) was **not** touched — Humanitarian incidents already publish without going through this hypothesis state machine, so it does not have the same gap; not yet separately verified.

Two more funnel contributors identified, not yet actioned (operator decision, not code): (1) `core/intel/gfw_monitor.py` (Global Fishing Watch — independent AIS/radar corroboration for grey-zone anomalies) is fully wired and already scheduled (`core/scheduler.py`) but is a no-op because `GFW_API_TOKEN` is unset — now documented in `ops/.env.production.example`; (2) `core/radio/catalog.py` already ships a pre-cleared 15-receiver public KiwiSDR/OpenWebRX catalog across the Mediterranean (`terms_status="allowed"`, generic public-receiver terms already declared) behind `REMOTE_RADIO_ENABLED=false` — also now documented in the same template. Both require operator action on the actual Oracle VM `.env` (not this repo, not Vercel — Vercel only hosts the static frontend); this session cannot flip either.
